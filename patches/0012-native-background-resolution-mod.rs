...
/// Whether the background may be rendered at a fraction of the pane's resolution on integrated GPUs.
/// Off: the resolution lever costs more than it saves (see the call site).
const SCALE_BACKGROUND: bool = false;
...
pub struct CustomShaderRenderer {
    ...
    slow_seconds: u32,
    /// Consecutive seconds spent above `FAST_FPS`, which is what buys resolution back.
    fast_seconds: u32,
    /// When the scale or the redraw rate last changed, so the policy cannot retune every second.
...
impl CustomShaderRenderer {
    ...
    pub fn render_with_clear_color(
        ...
        // The background is drawn at the pane's own resolution, every frame. Scaling it down on
        // integrated GPUs bought less than it looked like it should: the scaled path needs a
        // compositing pass of its own, so the saving is small next to that cost, and a softer
        // background is the first thing the eye notices. The machinery below stays for a machine
        // that genuinely cannot keep up, but it is off, and the native path draws straight into the
        // output view with no composite.
        let scalable = SCALE_BACKGROUND && self.integrated_gpu && !self.full_content_mode;
        // An integrated GPU also gets a background texture of its own, at the pane's own resolution.
        // Owning the texture is what allows redrawing the artwork less often than the pane is
        // presented: the composite that follows is a single textured quad, while the artwork behind it
        // is the expensive part. Measured on an integrated Radeon at 1573x941, drawing the same
        // native-resolution artwork every frame pins the engine at 105% and the whole terminal at
        // 29 fps, while reusing the texture lands far below that with the text still at 60.
        let owned = self.integrated_gpu && !self.full_content_mode;
        ...
        if scalable {
            ...
            if frame_rate_sampled {
                if self.current_frame_rate < scaled_background::SLOW_FPS {
                    self.slow_seconds = self.slow_seconds.saturating_add(1);
                    self.fast_seconds = 0;
                } else if self.current_frame_rate > scaled_background::FAST_FPS {
                    self.fast_seconds = self.fast_seconds.saturating_add(1);
                    self.slow_seconds = 0;
                } else {
                    // The band between the two thresholds is what stops a machine sitting on the fence
                    // from being retuned every single second.
                    self.slow_seconds = 0;
                    self.fast_seconds = 0;
                }
            }
            let cooled = self.last_scale_change.is_none_or(|last| {
                last.elapsed().as_secs_f32() >= scaled_background::COOLDOWN_SECONDS
            });
            let behind = self.slow_seconds >= scaled_background::SLOW_SECONDS && cooled;
            let ahead = self.fast_seconds >= scaled_background::FAST_SECONDS && cooled;
            let next = if behind && current < scaled_background::MAX_DIVISOR {
                self.slow_seconds = 0;
                scaled_background::coarser(current, floor)
            } else if ahead && current > floor {
                self.fast_seconds = 0;
                scaled_background::finer(current, floor)
            } else {
                current
            };
            ...
            } else if ahead && self.background_fps < scaled_background::DEFAULT_BACKGROUND_FPS {
                // Headroom goes back into the redraw rate first: it costs nothing in sharpness, so the
                // resolution is the last thing to come back.
                self.fast_seconds = 0;
                let faster =
                    (self.background_fps * 2.0).min(scaled_background::DEFAULT_BACKGROUND_FPS);
                log::info!(
                    "Background redraw rate: {:.0} -> {:.0} fps ({:.0} fps rendered)",
                    self.background_fps,
                    faster,
        ...
        // The artwork is a slow nebula, so redrawing it at half the display rate halves what the
        // shader costs while still reading as motion - and it costs nothing in sharpness, which is
        // why this is the lever that pays rather than scaling the background down. Measured on an
        // integrated Radeon at 1573x941: native resolution redrawn every frame pins the engine at
        // 105% and 29 fps, while the same resolution at 30 fps sits far below it with the terminal
        // still presenting at 60. A non-animated shader only needs a redraw when something else
        // changes, so it leans on the same texture for a second at a time. The shader draws straight
        // into the output view and the texture it leaves behind is reused.
