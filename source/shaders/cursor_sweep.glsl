...
vec2 getRectangleCenter(vec4 rectangle) {
    // y increases downward, so center = top + half_height
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float sdfCurrentCursor = getSdfRectangle(vu, centerCC, currentCursor.zw * 0.5);
     ...
     // Only draw trail while animation is active (not finished or nearly finished)
     if (lineLength > minDist && progress < 0.99) {
         // --- Animation Logic ---
        ...
        // Note: iCurrentCursor.xy is top-left corner, y increases downward in normalized coords
