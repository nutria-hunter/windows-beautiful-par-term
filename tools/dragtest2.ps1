...
Write-Output "window hwnd=$t  before L=$($r0.L) T=$($r0.T)"
$style = 0
Write-Output "menu=$([T]::GetMenu($t))"

# --- variant A: PostMessage (non-blocking) ---
$gx = $r0.L + 150; $gy = $r0.T + 3
[void][T]::SetCursorPos($gx, $gy)
Start-Sleep -Milliseconds 400
[T]::LDown()
Start-Sleep -Milliseconds 150
[void][T]::ReleaseCapture()
[void][T]::PostMessage($t, 0x00A1, [IntPtr]2, [IntPtr]0)   # WM_NCLBUTTONDOWN, HTCAPTION
Start-Sleep -Milliseconds 250
for ($i = 1; $i -le 8; $i++) {
  [void][T]::SetCursorPos($gx + [int]($Dx*$i/8), $gy + [int]($Dy*$i/8))
...
Write-Output "A PostMessage : after L=$($r1.L) T=$($r1.T)  delta=($($r1.L-$r0.L), $($r1.T-$r0.T))"
