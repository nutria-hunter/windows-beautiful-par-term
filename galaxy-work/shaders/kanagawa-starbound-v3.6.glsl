...
vec3 asteroidGroup(vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int j=0;j<26;j++) {
        ...
        vec2 rock = center - dir*(r1*0.30+progress*0.16)*size.x*0.45 + side*(r2-0.5)*0.30*size.y;
        vec2 d = pixel-rock;
...
vec2 incursionSchedule(float time) {
    ...
    if(seed<0.45) return vec2(-100.0,seed);
    float delay=20.0+200.0*hash21(vec2(slot,29.0));
...
vec3 alienHull(vec2 s, float kind, float age, float depth, vec3 bg) {
    ...
    if(hull) {
        ...
        if(kind>1.5 && sy>6.0 && sy<12.0 && mod(floor(s.x)+14.0,5.0)<2.0)
            shell=vec3(198,68,104)/255.0;
        c=shell*(0.66+0.34*depth);
    }
    float engineX=kind>1.5 ? -14.0 : (kind>0.5 ? -18.0 : -20.0);
...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, vec3 color) {
    ...
    float attack=(age>2.0 && age<life-2.4) ? 1.0 : 0.0;
    ...
    int count=3+int(floor(hash21(vec2(seed,71.0))*3.99));
    ...
    for(int j=0;j<6;j++) {
        ...
        float kind=mod(floor(hash21(vec2(seed*7.0,id+31.0))*3.0),3.0);
        ...
        if(attack>0.5) {
            ...
            // bubble and the glints land on it instead of on the city.
            float stopR = shieldStr>0.02 ? 1.06 : 1.0;
            vec2 toward=normalize(tgtPx-center);
            vec2 limb=tgtPx-toward*targetR*size.y*stopR;
    ...
    if(attack>0.5) {
        for(int m=0;m<3;m++) {
            ...
            vec2 shipPx=raiderPos(formation,forward,sidev,id,seed,size);
