...
vec3 shipEvent(vec2 fragCoord, vec3 bg) {
    ...
    for(int j=0;j<min(count,18);j++) {
        ...
        float easeRelease=1.0-((1.0-release)*(1.0-release)*(1.0-release)*(1.0-release));
        float arrivalOffset=mix(-0.72,-0.95,load);
        if(arrive>=0.28) arrivalOffset=mix(-0.95,0.16,easeRelease);
        if(arrive>=0.48) arrivalOffset=0.16*((1.0-settle)*(1.0-settle)*(1.0-settle));
        float launch=clamp((depart-0.30)/0.70,0.0,1.0);
        float recoil=-0.12*sin(min(depart/0.30,1.0)*1.5707963);
        float exitOffset=recoil+3.2*((launch)*(launch)*(launch)*(launch));
        ...
        float widthScale=mix(0.90,1.0,easeRelease)*max(0.15,1.0-((depart)*(depart)*(depart)*(depart)*(depart)));
