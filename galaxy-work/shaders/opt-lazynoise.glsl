...
vec3 planet(vec2 p, vec2 center, float radius, float kind, vec3 bg, vec2 pixel) {
    ...
    vec3 c;
    if (kind < 0.5) {
        // Gas giant: banded and cool, with one oval storm. The cloud noise is only this family's,
        // so it is sampled here instead of for every planet pixel.
        ...
        // where a volcanic world actually shows its own light. Its basalt noise is sampled here for
        // the same reason as the gas giant's clouds.
