...
vec3 patrolPass(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int k=0;k<3;k++) {
        ...
        color=fleetHull(d/0.60,kind,age*0.5+id,0.6,color);
    }
    return color;
}
    ...
    if(hash21(vec2(seed,31.0))<0.5) {
        ...
        for(int w=0;w<2;w++) {
            ...
            if(abs(q.x)>r*0.30 && abs(q.x)<px1 && abs(q.y)<py) {
                ...
                color=max(color, q.x<0.0 ? vec3(40,58,88)/255.0 : vec3(56,78,112)/255.0);
...
vec3 asteroidGroup(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int j=0;j<26;j++) {
        ...
        // A few of the big rocks are being worked: metal body, an ore seam, and drones on station. The ore
        // uses the palette's own accents - gold, silver, and the Kanagawa crystal blue.
        bool mined = r3>0.86 && hash21(vec2(seed*53.0,id+23.0))>0.42;
...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, float railShieldStr, vec3 color) {
    ...
    for(int j=0;j<9;j++) {
        ...
        vec2 p2=floor((fragCoord-floor(center*px)/px)/px)+0.5;
        ...
        for(int b=0;b<3;b++) {
            ...
            if(bAge<0.42) {
                ...
                vec2 relF=fragCoord-mix(firingPos,hitPt,bAge/0.42)*px;
                float alongF=dot(relF,dirB);
                if(abs(alongF)<7.0 && length(relF-dirB*alongF)<0.6)
                ...
                if(((railHit ? railShieldStr : shieldStr)>0.02) && !duelling) {
                    ...
                    float hr=1.2-0.8*fade;
                    float edge=hexEdge(rel,hr);
                    if(hr>0.4) {
                        if(edge<0.8) color=mix(color,vec3(168,224,190)/255.0,0.50*(1.0-fade*0.6));
                        else if(edge<1.4 && fade<0.45)
    ...
    if(attack>0.5) {
        for(int m=0;m<3;m++) {
            ...
            for(int q2=0;q2<2;q2++) {
                ...
                vec2 relF=fragCoord-mix(stPx,shipPx,phase/0.55)*px;
                float alongF=dot(relF,toward);
                if(abs(alongF)<9.0 && length(relF-toward*alongF)<0.6)
                    color=max(color,vec3(186,244,252)/255.0);
                // Muzzle flash: a small glare at the battery as the bolt leaves.
                if(phase<0.12 && abs(fragCoord.x-stPx.x*px)<2.0 && abs(fragCoord.y-stPx.y*px)<2.0)
    ...
    if(defended && dCount>0) {
        ...
        for(int k=0;k<5;k++) {
            ...
            if(age<down) {
                vec2 p3=floor((fragCoord-floor(dcenter*px)/px)/px)+0.5;
                ...
                if(fAge<0.45 && count>0 && dIn>0.85) {
                    ...
                    vec2 relF2=fragCoord-mix(dcenter,tp,fAge/0.45)*px;
                    float a2=dot(relF2,tw);
                    if(tgtOk && abs(a2)<7.0 && length(relF2-tw*a2)<0.6 && age<raiderDownTime(float(tgt),seed))
                ...
                if(hitAge>0.0 && hitAge<0.25 && count>0 && dIn>0.85 && age>3.8+0.45*float(impTgt)) {
                    int tgt=int(floor(hash21(vec2(seed,149.0+id))*float(count)));
                    vec2 tp=snapFine(raiderPos(formation,forward,sidev,float(tgt),seed,size));
                    vec2 rel3=pixel-tp;
...
vec3 shipEvent(vec2 fragCoord, vec3 bg) {
    ...
    for(int j=0;j<18;j++) {
        ...
        vec2 pixel=floor((fragCoord-floor(center*px)/px)/px)+0.5;
