...
// Every body crosses the sky left to right, the way a star system's rotation reads from inside it,
// and wraps while it is still well off-screen so the loop never shows. `spanScale` stretches the
// travel path: a small factor keeps a body in frame for much of its cycle (the hero world), a large
// one turns it into a rare visitor - which is how the scene avoids piling every planet onto the
// screen at once. The speed is scaled with the span, so the on-screen pace stays calm either way.
vec2 orbitLane(float aspect, float phase, float rate, float amp, float mid, float height, float spanScale) {
    float span=(aspect+2.0*amp)*spanScale;
    float start=fract(phase/TAU);
    float x=mod(start+iTime*iOrbitSpeed*rate*0.006/spanScale, 1.0)*span-(amp+0.2);
    float y=mid+height*sin(phase+iTime*iOrbitSpeed*rate*0.05+x*0.6);
    return vec2(x,y);
}
vec2 orbit(float aspect, float phase, float rate, float height) {
    return orbitLane(aspect, phase, rate, 0.78, 0.5, height, 1.0);
}
...
vec3 cityPlanet(vec2 q, vec2 center) {
    ...
    // Quantised lighting of that one surface: five steps, so the poster look survives while the
    // features themselves run continuously. The night half keeps a cold cast rather than a flat
    // darkening, which is what the terminator was missing.
    float lit = band<0.5 ? 0.20 : (band<1.5 ? 0.46 : (band<2.5 ? 0.70 : (band<3.5 ? 0.88 : 1.0)));
    vec3 c = albedo*lit;
    if(band<1.5) c=mix(c, c*vec3(0.78,0.90,1.15), 0.55);
    c=max(c, lights);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec2 center = orbitLane(aspect,1.2,1.0,0.78,0.50,0.25,1.0);    // hero: in frame about half its cycle
    vec2 gas = orbitLane(aspect,4.10,0.87,0.62,0.30,0.16,3.2);      // upper lane, rare visitor
    vec2 ochre = orbitLane(aspect,2.30,0.73,0.86,0.72,0.20,3.8);    // lower lane
    vec2 ice = orbitLane(aspect,5.40,1.35,0.55,0.14,0.10,4.4);      // high and quicker
    vec2 lava = orbitLane(aspect,0.60,0.55,0.70,0.86,0.10,5.0);     // slowly along the bottom edge
    // A world far away, slow and only briefly in frame. Drawn first and hazed, so every other body
    // crosses in front of it.
    vec2 farBody = orbitLane(aspect,3.30,0.14,0.30,0.60,0.08,2.4);
    ...
    color=planet(screen,farBody,0.042,4.0,color);
    ...
    color=planet(screen,ice,0.095,2.0,color);
    color=planet(screen,lava,0.062,3.0,color);
    ...
    // The star field is the background, so it is painted here - before the near bodies - and a planet
    // then occludes the stars with its own disk. The old version excluded stars inside a radius of
    // 1.75 planet radii instead, which left a starless halo around every body that read as a shadow
    // cast onto the sky. The distant world above is the deliberate exception: it is far enough that
    // stars pass in front of it, which is what sells its distance.
    {
