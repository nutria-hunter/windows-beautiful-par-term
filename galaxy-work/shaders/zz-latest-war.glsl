...
vec3 warPass(vec2 fragCoord, vec2 center, float planetR, vec3 color) {
    ...
    for(int i=0;i<6;i++) {
        ...
        if(abs(d.x)>7.0 || abs(d.y)>3.0) continue;
        color=max(color,vec3(38,44,54)/255.0);
        if(abs(d.x)<1.7 && abs(d.y)<1.0) color=max(color,vec3(96,214,214)/255.0);
    }
...
void renderOperaScene(out vec4 fragColor, in vec2 fragCoord) {
    ...
    color=warPass(fragCoord,orbitLane(aspect,1.2,1.0,0.78,0.50,1.0),CITY_R,color);
