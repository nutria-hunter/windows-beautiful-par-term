...
author: Codex
description: Flat retro space mural, ink-blue nebula banks, cut-paper planets and quiet pixel stars. Texture-free; banded day/night lighting with a real terminator, restrained lit-limb atmospheres, ring shadows, coast-hugging city lights, layered star drifts and nebula banks that slowly breathe.
version: 1.8.0
defaults:
  animation_speed: 1.0
  brightness: 1.0
...
vec3 cityPlanet(vec2 q, vec2 center) {
    ...
    // Day side: the same continents that organise the night lights have to be visible under
    // sunlight, or the lit hemisphere is a flat wash. Ocean, land, a shelf line and a cloud deck.
    if(band>1.5) {
        if(land>0.60) c+=vec3(12,14,13)/255.0;
        if(nearCoast) c=mix(c,vec3(58,74,86)/255.0,0.30);
        if(cloud>0.62) c=mix(c,vec3(76,88,96)/255.0,0.36);
    }
    ...
    if(road || tower || metropolis>0.0) {
        ...
        if(band>1.5) {
            // By day the same districts read as grey sprawl instead of lights, which is what gives
            // the lit hemisphere structure and ties it to the night map on the other side.
            float sprawl = metropolis>0.0 ? 0.42 : (tower ? 0.26 : 0.18);
            c=mix(c,vec3(70,76,82)/255.0,sprawl*iCityGain);
        } else {
