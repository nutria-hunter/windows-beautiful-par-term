...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, float railShieldStr, vec3 color) {
    ...
    float lane=clamp(target.y+side*(0.24+0.16*hash21(vec2(seed,23.0))),0.10,0.90);
      // A raid only ever touches two places: the sliver along its lane and the world it is firing
      // at. Everything else on screen skips the squadron, its gates, its shields and its gunfire -
      // one comparison against the two bands instead of nine ships' worth of geometry.
      float laneSliver=0.20*size.y+90.0*px;
      float worldReach=1.7*targetR*size.y+90.0*px;
      bool nearLane=abs(fragCoord.y-lane*size.y)<=laneSliver;
      bool nearWorld=abs(fragCoord.y-target.y*size.y)<=worldReach;
      if(!nearLane && !nearWorld) return color;
