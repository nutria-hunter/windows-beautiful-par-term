...
vec3 structurePass(vec2 pixel, vec2 size, vec3 color) {
    ...
    if(hash21(vec2(seed,31.0))<0.5) {
        ...
        // The mouth: a vortex of dithered ink, drifting inward.
        if(rr<r*0.86) {
            float swirl=0.5+0.5*sin(ang*2.0-rr/max(r,1.0)*6.0+iTime*0.18+seed*9.0);
            float li=ditherLevel(0.14+0.34*swirl,pixel,5.0);
            color=max(color,mix(vec3(20,23,32)/255.0,vec3(74,96,124)/255.0,li));
        }
        ...
        for(int k=0;k<5;k++) {
            ...
            if(age<down) {
                ...
                if(fAge<0.45 && count>0 && dIn>0.85) {
                    ...
                    if(abs(a2)<7.0 && length(relF2-tw*a2)<0.6 && age<raiderDownTime(float(tgt),seed))
