...
vec2 shipSchedule(float time) {
    ...
    // LIVE TEST BUILD: the fleet is always mid-event, cycling in/hold/out every 40 seconds, so the
    // warp-in, the ships and the warp-out can actually be watched instead of waited for. Restore
    // kanagawa-starbound-normal-schedule.glsl afterwards.
    return vec2(mod(clock,40.0),0.75);
    if(seed<0.18) return vec2(-100.0,seed);
...
vec3 shipEvent(vec2 fragCoord, vec3 bg) {
    ...
    for(int j=0;j<18;j++) {
        ...
        float arrive=clamp(t/1.1,0.0,1.0);
        ...
        float lengthScale=mix(0.45,1.0,arrive)*max(0.03,1.0-depart*1.02);
        float widthScale=mix(0.12,1.0,arrive)*max(0.02,1.0-depart*1.05);
        ...
        if(abs(s.x)<900.0 && abs(s.y)<900.0) {
            vec3 hull=warshipArt(s,kind,t+rnd,depth,color,armorLight(center/iResolution.y,forward),false);
            if(any(greaterThan(abs(hull-color),vec3(0.002)))) color=max(color,hull*1.85);
        }
    }
    return color;
}
