...
vec3 cometPixel(vec2 pixel, vec2 size, vec3 color) {
    ...
    vec2 head=snapPixel(mix(start,endp,progress)*size.y);
    ...
    float back=-along;
    float tailLen=34.0+18.0*hash21(vec2(seed,37.0));
    if(back>0.0 && back<tailLen && length(rel-dir*along)<3.0-0.06*back) {
...
vec3 meteorPixel(vec2 pixel, vec2 size, vec3 color) {
    ...
    // Trail: one logical pixel wide and only a dozen or two long, in two flat tones that fade back.
    float along=dot(rel,dir);
    float back=-along;
    float tail=9.0+13.0*hash21(vec2(seed,37.0));
    if(back>0.0 && back<tail && length(rel-dir*along)<0.9) {
        vec3 tone = back<tail*0.45 ? vec3(186,206,228)/255.0 : vec3(98,120,148)/255.0;
...
// the fast traffic. Its own long slot, so it is a landmark that happens to pass by.
vec2 structureSchedule(float time) {
    if(iWarpRate<=0.0) return vec2(-100.0,0.0);
    float clock=time*iWarpRate;
    float slot=floor(clock/240.0);
    float seed=hash21(vec2(slot,3079.0));
    if(seed<0.50) return vec2(-100.0,seed);
    float delay=10.0+170.0*hash21(vec2(slot,41.0));
    return vec2((mod(clock,240.0)-delay)/iWarpRate,seed);
}
vec3 structurePass(vec2 pixel, vec2 size, vec3 color) {
    ...
    float life=110.0+60.0*hash21(vec2(seed,53.0));
    if(age>life) return color;
    float progress=age/life;
    float lane=0.12+0.76*hash21(vec2(seed,11.0));
    vec2 center=snapPixel(mix(vec2(-0.12,lane),vec2(aspect+0.12,lane),progress)*size.y
        +vec2(0.0,sin(progress*6.2831853)*0.008*size.y));
    float r=(0.030+0.020*hash21(vec2(seed,23.0)))*size.y;   // logical pixels
    vec2 d=pixel-center;
    ...
    if(hash21(vec2(seed,31.0))<0.5) {
        ...
        // Station: a spine with a hub, two solar wings, a habitation ring and lit windows.
        if(abs(d.x)<r*0.11 && abs(d.y)<r*1.05) color=max(color,vec3(82,96,108)/255.0);
        if(abs(d.x)<r*0.58 && abs(d.y)<r*0.17) color=max(color,vec3(102,116,126)/255.0);
        if(abs(d.x)>r*0.32 && abs(d.x)<r*1.45 && abs(abs(d.y)-r*0.55)<r*0.27) {
            vec3 panel=vec3(36,54,82)/255.0;
            if(mod(floor(d.x/2.0)+floor(d.y/2.0),2.0)<1.0) panel=vec3(50,72,104)/255.0;
            color=max(color,panel);
        }
        if(abs(length(d)-r*0.66)<1.7) color=max(color,vec3(122,136,146)/255.0);
        if(abs(d.x)<r*0.11 && abs(d.y)<r*1.05 && hash21(floor(d/2.0)+vec2(seed*13.0))>0.74)
            color=max(color,vec3(198,176,120)/255.0);
    }
    return color;
}
...
vec3 asteroidGroup(vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int j=0;j<26;j++) {
        ...
        vec2 rock = snapPixel(center - dir*(r1*0.30+progress*0.16)*size.x*0.45 + side*(r2-0.5)*0.30*size.y);
        vec2 d = pixel-rock;
        ...
        // Irregular silhouette: a lumpy circle whose edge is sampled on the logical pixel grid, so the
        // outline stays a staircase of hard blocks instead of a smooth blob.
        float lump = noise2(floor(d/2.5)+vec2(id*3.7,id*1.9));
        float rr = wr*(0.72+0.50*lump);
        float dd = dot(d,d);
        if(dd > rr*rr) continue;
        // Shading: sun from the upper left, a dark underside, and a lit rim along the terminator.
        float g = d.x*0.7 + d.y*1.2;
        vec3 tone = vec3(96,92,84)/255.0;
        if(g < -wr*0.30) tone = vec3(134,128,116)/255.0;
        else if(g > wr*0.35) tone = vec3(52,50,48)/255.0;
        if(dd > (rr-1.8)*(rr-1.8) && g < 0.0) tone = vec3(158,150,134)/255.0;   // rim light
        if(hash21(floor(d)+vec2(id*11.0,id*5.0)) > 0.88) tone = vec3(40,38,38)/255.0;  // craters
        color=max(color,tone);
    }
    return color;
}
// Forward declarations: the incursion code below uses the fleet's own hulls and gates, which are
// defined further down with the rest of the fleet.
...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, vec3 color) {
    float aspect=size.x/size.y;
    vec2 ev=incursionSchedule(iTime);
    float age=ev.x, seed=ev.y;
    if(age<0.0) return color;
    float life=13.0+4.0*hash21(vec2(seed,61.0));
    if(age>=life) return color;
    float px=max(floor(iPixelSize),1.0);
    ...
    float inP=clamp(age/2.0,0.0,1.0);
    float outP=clamp((age-(life-2.4))/2.4,0.0,1.0);
    ...
    if(!planetVisible) life=min(life,6.5);
    float attack=(planetVisible && age>2.0 && age<life-2.4) ? 1.0 : 0.0;
    float travel=clamp((age-2.0)/max(0.1,life-4.4),0.0,1.0);
    ...
    vec2 dFormation=mix(dEntry,dExit,clamp((age-2.6)/max(0.1,life-4.0),0.0,1.0))*size.y;
    for(int j=0;j<9;j++) {
        if(j>=count) continue;
        float id=float(j);
        // Shot-down raiders are gone before they can fire again.
        if(age>raiderDownTime(id,seed)) continue;
        vec2 center=snapPixel(raiderPos(formation,forward,sidev,id,seed,size));
        // Gates: they open where the squadron entered, and again where it leaves.
        float gatePhase=age<life*0.5 ? inP : outP;
        ...
        float lenS=mix(0.10,1.0,inP)*(1.0+outP*2.2);
        float widS=mix(0.14,1.0,inP)*max(0.05,1.0-outP);
        ...
        if(attack>0.5) {
            ...
            vec2 limb=aimPx-toward*aimRadius;
            float boltAge=mod(age-id*0.19,1.15);
            if(boltAge<0.50) {
                // Bolt: a long hot segment travelling from the hull to the target, so the line of fire
                // is unmistakable even at a glance.
                vec2 ppos=mix(center,limb,boltAge/0.50);
                vec2 rel=pixel-ppos;
                float along=dot(rel,toward);
                if(abs(along)<7.0 && length(rel-toward*along)<1.5)
                    color=max(color,vec3(242,148,150)/255.0);
            } else if(boltAge<0.78) {
                // Impact: a hard burst that shrinks as it fades. A cold shield glint when the bubble
                // takes it, a hot burst when it reaches the ground.
                vec2 rel=pixel-limb;
                float fade=(boltAge-0.50)/0.28;
                float w=5.0-fade*9.0;
                if(w>0.4) {
                    vec3 hitCol = shieldStr>0.02 ? vec3(198,236,246)/255.0 : vec3(248,206,196)/255.0;
                    vec3 ringCol = shieldStr>0.02 ? vec3(124,196,222)/255.0 : vec3(226,116,124)/255.0;
                    if(abs(rel.x)<=w && abs(rel.y)<=w) color=max(color,hitCol);
                    float ring=w+1.6;
                    if(abs(max(abs(rel.x),abs(rel.y))-ring)<1.2 && fade<0.5)
                        color=max(color,ringCol);
                }
            }
        }
    }
    // Counter-battery: the orbital ring answers. Station m shoots down raider m, so the volley visibly
    // thins out as the incursion runs.
    if(attack>0.5) {
        for(int m=0;m<3;m++) {
            if(m>=count) continue;
            float id=float(m);
            float down=raiderDownTime(id,seed);
            vec2 e=stationPoint(0.65+float(m)*TAU/3.0+iTime*iCityFlow*0.022);
            vec2 stLocal=vec2(e.x-0.30*e.y,e.y+0.30*e.x)*1.09;
            vec2 stPx=target*size.y+stLocal*targetR*size.y;
            vec2 shipPx=snapPixel(raiderPos(formation,forward,sidev,id,seed,size));
            float bAge=age-(down-0.45);
            if(bAge>0.0 && bAge<0.45) {
                vec2 toward=normalize(shipPx-stPx);
                vec2 rel=pixel-mix(stPx,shipPx,bAge/0.45);
                float along=dot(rel,toward);
                if(abs(along)<5.0 && length(rel-toward*along)<1.3)
                    color=max(color,vec3(152,236,244)/255.0);
            }
            ...
            if(eAge>0.0 && eAge<0.60) {
                ...
                float w=7.0-fade*8.0;
                if(w>0.4 && abs(rel.x)<=w && abs(rel.y)<=w)
    ...
    if(defended && dCount>0) {
        ...
        for(int k=0;k<5;k++) {
            ...
            vec2 dcenter=snapPixel(defenderPos(dFormation,dForward,dSide,id,seed,size));
            ...
            if(age<down) {
                ...
                if(fAge<0.45 && count>0) {
                    ...
                    vec2 tp=snapPixel(raiderPos(formation,forward,sidev,float(tgt),seed,size));
                    vec2 tw=normalize(tp-dcenter);
                    vec2 rel2=pixel-mix(dcenter,tp,fAge/0.45);
                    float a2=dot(rel2,tw);
                    if(abs(a2)<6.0 && length(rel2-tw*a2)<1.4)
                ...
                if(d2<0.6) {
                    ...
                    float fade2=d2/0.6;
                    float w2=7.0-fade2*8.0;
                    if(w2>0.4 && abs(rel3.x)<=w2 && abs(rel3.y)<=w2)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(abs(q.x)<1.8 && abs(q.y)<1.5) {
        ...
        // Shield: a hard ring just outside the limb that brightens as it charges and dims as it strains.
        // The break itself is an expanding ring, so the failure reads without any particle effects.
        if(qlen2>0.90 && qlen2<1.60) {
            float rr=sqrt(qlen2);
            if(shield.x>0.02 && abs(rr-1.06)<1.5*pixelWidth) {
                vec3 st=mix(vec3(52,88,116)/255.0, vec3(132,204,220)/255.0, shield.x);
                color=max(color,st*(0.45+0.55*shield.x));
            }
            if(shield.y>0.0 && abs(rr-(1.06+(1.0-shield.y)*0.10))<1.6*pixelWidth)
                color=max(color,vec3(226,246,250)/255.0*shield.y);
        }
    }
    color*=iSceneGain;
    ...
    color=incursionEvent(fragCoord,pixel,size,center,CITY_R,shield.x,color);
