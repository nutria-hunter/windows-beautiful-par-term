...
author: Codex
description: Flat retro space mural, ink-blue nebula banks, cut-paper planets and quiet pixel stars. Texture-free; restrained lit-limb atmospheres, ring shadows, coast-hugging city lights, layered star drifts and nebula banks that slowly breathe.
version: 1.7.0
defaults:
  animation_speed: 1.0
  brightness: 1.0
...
vec3 planet(vec2 p, vec2 center, float radius, float kind, vec3 bg) {
    ...
    if (r2 > 1.0 && lightSide > 0.15)
        return max(bg, vec3(34,56,70)/255.0*(iRim*(lightSide > 0.55 ? 1.0 : 0.45)));
    ...
    if(r2>0.94 && lightSide>0.50) c=mix(c,vec3(89,109,117)/255.0,0.30*iRim);
    if (kind > 0.5) {
    ...
    // The inner half of the same atmosphere: a slight lift rather than a highlight.
    if (r2 > 1.0 - 2.0*limb && lightSide > 0.35) c = mix(c, vec3(66,92,106)/255.0, 0.40*iRim);
    return c;
}
vec3 cityPlanet(vec2 q, vec2 center) {
    ...
    // The same restrained limb treatment as the natural planets (see `planet`).
    if(dot(q,q)>0.975 && light>0.10) c=mix(c,vec3(58,86,100)/255.0,0.45*iRim);
    if(dot(q,q)>0.990 && light>0.30) c=mix(c,vec3(76,106,120)/255.0,0.35*iRim);
    return c;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(abs(q.x)<1.8 && abs(q.y)<1.5) {
        ...
        if(qlen2>1.0 && qlen2<halo*halo) {
            ...
            if(cl>0.15) color=max(color,vec3(34,56,70)/255.0*(iRim*(cl>0.55 ? 1.0 : 0.45)));
        }
