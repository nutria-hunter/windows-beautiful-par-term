...
vec3 patrolPass(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int k=0;k<3;k++) {
        ...
        vec2 d=fineLocal(fragCoord,p2p,px);
...
vec3 structurePass(vec2 pixel, vec2 size, vec3 color) {
    ...
    if(hash21(vec2(seed,31.0))<0.5) {
        // Stargate: a segmented outer ring with nine bright chevrons, an inner track, a hub ring, a
        // dithered iris that breathes, six slow vanes inside the mouth, support towers below and a
        // beacon. Colours are Kanagawa's own: katanaGray and sumiInk for structure, crystalBlue for the
        // field, springViolet for the vanes, waveRed for lights.
        float rr=length(d);
        float ang=atan(d.y,d.x);
        float seg=mod(floor(ang*5.7275649),2.0);                       // nine divisions
        if(abs(rr-r)<2.2) color=max(color, seg<1.0 ? vec3(127,127,127)/255.0 : vec3(68,68,88)/255.0);
        if(abs(rr-r*0.80)<1.3) color=max(color,vec3(126,156,216)/255.0);
        if(abs(rr-r*0.42)<1.2) color=max(color,vec3(88,100,130)/255.0);
        // Chevrons: a bright block in every ninth of the ring.
        if(abs(rr-r)<4.2 && mod(ang*5.7275649,1.0)<0.15) color=max(color,vec3(200,200,200)/255.0);
        // Iris: layered dithered light, brightest at the hub, breathing slowly.
        if(rr<r*0.40) {
            float li=ditherLevel(0.30+0.35*(0.5+0.5*sin(iTime*0.5+seed*9.0)),pixel,5.0);
            color=mix(color,mix(vec3(31,31,40)/255.0,vec3(126,156,216)/255.0,li),0.90);
        } else if(rr<r*0.76) {
            float li=ditherLevel(0.18+0.30*(0.5+0.5*sin(iTime*0.5+seed*9.0)),pixel,4.0);
            color=mix(color,mix(vec3(26,26,35)/255.0,vec3(88,100,130)/255.0,li),0.85);
        }
        // Six slow vanes inside the mouth.
        if(rr<r*0.74 && rr>r*0.38 && abs(sin(ang*3.0+iTime*0.12+seed*5.0))>0.985)
            color=max(color,vec3(149,127,166)/255.0);
        // Support towers: a central pylon with outriggers and a cross-brace.
        if(abs(d.x)<1.8 && d.y>r*0.94 && d.y<r*1.36) color=max(color,vec3(68,68,88)/255.0);
        if(abs(abs(d.x)-r*0.52)<1.5 && d.y>r*0.62 && d.y<r*1.12) color=max(color,vec3(54,54,70)/255.0);
        if(abs(d.y-r*0.86)<1.1 && abs(d.x)<r*0.54) color=max(color,vec3(127,127,127)/255.0);
        if(length(d-vec2(0.0,r*1.40))<2.0 && 0.5+0.5*sin(iTime*1.9+seed*7.0)>0.5)
            color=max(color,vec3(195,64,67)/255.0);
        // Navigation pair on the flanks, Kanagawa red and winter green.
        if(abs(abs(d.x)-r)<1.6 && abs(d.y)<1.6)
            color=max(color, d.x>0.0 ? vec3(195,64,67)/255.0 : vec3(106,149,137)/255.0);
    } else {
...
vec3 asteroidGroup(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int j=0;j<26;j++) {
        ...
        vec2 d = fineLocal(fragCoord,rock,px);
        ...
        if(mined) {
            ...
            for(int k=0;k<3;k++) {
                ...
                vec2 dq=fineLocal(fragCoord,dp,px);
    ...
    if(attack>0.5) {
        for(int m=0;m<3;m++) {
            ...
            for(int q2=0;q2<2;q2++) {
                ...
                if(abs(alongF)<36.0 && length(relF-toward*alongF)<0.8)
        ...
        for(int k=0;k<5;k++) {
            ...
            if(age<down) {
                ...
                if(fAge<0.45 && count>0 && dIn>0.85) {
                    ...
                    if(abs(a2)<8.0 && length(rel2-tw*a2)<0.5 && age<raiderDownTime(float(tgt),seed))
                ...
                if(hitAge>0.0 && hitAge<0.25 && count>0 && dIn>0.85) {
                    int tgt=int(floor(hash21(vec2(seed,149.0+id))*float(count)));
                    vec2 tp=snapFine(raiderPos(formation,forward,sidev,float(tgt),seed,size));
                    vec2 rel3=pixel-tp;
