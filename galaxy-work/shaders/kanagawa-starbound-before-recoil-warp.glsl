...
vec3 metricWake(vec2 q, float phase, float depth, vec3 bg, bool alien) {
    ...
    for(int j=0;j<3;j++) {
        ...
        float front=curve+(2.0+k*2.5)*d;
        float rear=-curve-(3.0+k*(4.0+phase*5.0))*d;
...
vec3 alienGate(vec2 q, float phase, float depth, vec3 bg) {
    return metricWake(q,clamp(phase*3.0,0.0,1.0),depth,bg,true);
}
    ...
    if(defended && dCount>0) {
        ...
        for(int k=0;k<5;k++) {
            ...
            if(abs(gd.x)<gspan && abs(gd.y)<gspan) {
                ...
                float shortPhase=age<life*0.5 ? clamp(dIn*4.0,0.0,1.0) : clamp(dOut*3.0,0.0,1.0);
...
vec3 shipEvent(vec2 fragCoord, vec3 bg) {
    ...
    for(int j=0;j<18;j++) {
        ...
        if(t<=0.0 || t>=cruiseEnd+0.75) continue;
        ...
        float arrive=clamp(t/0.85,0.0,1.0);
        float depart=clamp((t-cruiseEnd)/0.75,0.0,1.0);
        center=mix(center,exitPoint,depart);
        // Short inertial kick on emergence; accelerating draw into the departing bubble.
        float kick=px*depth*9.0;
        center+=forward*kick*(-pow(1.0-arrive,3.0)+depart*depart);
        center=floor(center);
