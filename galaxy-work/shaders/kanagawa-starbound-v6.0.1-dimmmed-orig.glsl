...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(gr.y<0.27*gr.x) color=ring(gr,color,gasLight);
    ...
    if(abs(q.x)<1.8 && abs(q.y)<1.5) {
        ...
        color=infrastructure(railQ,railFine,color,0.0,pixelWidth);
        if(qlen2<=1.0) {
        ...
        color=infrastructure(railQ,railFine,color,1.0,pixelWidth);
