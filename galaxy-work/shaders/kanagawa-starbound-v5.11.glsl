...
vec3 structurePass(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    vec2 d=floor(fineLocal(fragCoord,center,px));
...
vec3 alienHull(vec2 s, float kind, float age, float depth, vec3 bg) {
    ...
    vec3 c=bg;
    if(hull) {
        vec3 shell=vec3(44,34,52)/255.0;
        if(s.y<0.0) shell=vec3(102,70,106)/255.0;
        if(sy<2.0) shell=vec3(148,96,124)/255.0;
        if(mod(floor(s.x)+30.0,7.0)<1.0 && sy>2.0) shell=vec3(34,24,42)/255.0;
        ...
        vec3 accent=vec3(195,64,67)/255.0;
        if(kind>0.5 && kind<1.5) accent=vec3(186,112,72)/255.0;
        else if(kind>1.5 && kind<2.5) accent=vec3(149,127,166)/255.0;
        else if(kind>2.5 && kind<3.5) accent=vec3(176,134,72)/255.0;
        else if(kind>3.5) accent=vec3(126,156,216)/255.0;
        if(s.x>-6.0 && s.x<10.0 && sy<2.5 && mod(floor(s.x),3.0)<1.0)
            shell=accent;
        if(kind>3.5 && sy>11.0 && sy<17.0 && mod(floor(s.x)+14.0,5.0)<2.0)
            shell=mix(accent,vec3(220,215,186)/255.0,0.4);
        ...
        if(sy<1.0) shell=vec3(148,98,122)/255.0;
        else if(s.y>0.0 && sy>2.0) shell=mix(shell,vec3(34,24,42)/255.0,0.45);
        if(sy>1.2 && sy<2.6 && mod(floor(s.x)+40.0,5.0)<1.0) shell=vec3(172,102,118)/255.0;
        if(mod(floor(s.x)+22.0,11.0)<1.0 && sy>3.0) shell=vec3(40,28,50)/255.0;
    ...
    if(s.x<engineX && s.x>engineX-flame*1.6 && (abs(sy-2.0)<1.4 || abs(sy-6.0)<1.4))
        c=max(c,vec3(186,112,72)/255.0*depth);
    return c;
}
...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, float railShieldStr, vec3 color) {
    ...
    for(int j=0;j<9;j++) {
        ...
        // A raid leaves the way it came: the squadron shrinks away into its own gate rather than swelling
        // as it exits, so the last thing seen is a distant pinch of light.
        float lenS=mix(0.10,1.0,inS)*max(0.05,1.0-outP*0.92);
        float widS=mix(0.16,1.0,inS)*max(0.03,1.0-outP*0.95);
    ...
    if(defended && dCount>0) {
        ...
        for(int k=0;k<5;k++) {
            ...
            if(age<down) {
                ...
                float lenS=mix(0.12,1.0,dIn)*max(0.05,1.0-dOut*0.92);
                float widS=mix(0.16,1.0,dIn)*max(0.03,1.0-dOut*0.95);
