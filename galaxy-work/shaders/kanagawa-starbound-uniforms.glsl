...
void renderOperaScene(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // DEBUG BUILD (diagnostic only): paint the uniforms this frame actually received instead of the
    // scene, so a screenshot reads the values back. Five horizontal bands, top to bottom:
    // iBrightness, iSceneGain, iNebula, iStarGain, iNebulaLife - each clamped to 0..1 as a grey level.
    {
        float band=fragCoord.y/iResolution.y;
        float v = band<0.20 ? iBrightness
                : band<0.40 ? iSceneGain
                : band<0.60 ? iNebula
                : band<0.80 ? iStarGain
                : iNebulaLife;
