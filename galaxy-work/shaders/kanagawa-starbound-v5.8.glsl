...
// Ships sit between the planets' 4px blocks and the fragment: a 2px lattice keeps the plating readable while
// matching the mural's chunkiness, and - sampled per fragment - it still translates without shimmering.
vec2 chunkLocal(vec2 fragCoord, vec2 center, float px) {
    return placeAt(fragCoord,center,px,2.0);
}
...
vec3 patrolPass(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int k=0;k<3;k++) {
        ...
        float dep=0.26+0.30*hash21(vec2(seed,73.0));
...
vec3 alienHull(vec2 s, float kind, float age, float depth, vec3 bg) {
    ...
    if(kind<0.5) {
        ...
        // Dagger wedge, the way a Star Destroyer reads: a long triangular hull that widens aft, with a
        // raised bridge tower over it and a shallow hangar notch in the belly.
        hull=(s.x>=-34.0 && s.x<=34.0 && sy<=max(1.2,(34.0-s.x)*0.26))
            || (s.x>-22.0 && s.x<-6.0 && sy>4.0 && sy<9.0)
            || (s.x>-18.0 && s.x<6.0 && sy>9.0 && sy<12.0)
            || (s.x>-6.0 && s.x<14.0 && sy<2.0);
    } else {
