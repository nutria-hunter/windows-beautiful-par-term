        ...
        for(int b=0;b<3;b++) {
            ...
            if(bAge<0.42) {
                ...
                if(((railHit ? railShieldStr : shieldStr)>0.02) && !duelling) {
                    // Small and semi-transparent: the shield marks the point of contact, it does not draw
                    // a bubble around the world.
                    float hr=2.6-1.8*fade;
                    float edge=hexEdge(rel,hr);
                    if(hr>0.6) {
                        if(edge<0.9) color=mix(color,vec3(168,224,190)/255.0,0.55*(1.0-fade*0.6));
                        else if(edge<2.2 && fade<0.45)
                            color=mix(color,vec3(70,132,110)/255.0,0.28*(1.0-fade));
                    }
                } else {
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(abs(q.x)<1.8 && abs(q.y)<1.5) {
        ...
        if(shieldRail.x>0.02) {
            ...
            if(hx<0.022) color=mix(color,vec3(150,212,182)/255.0,0.42*shieldRail.x);
            else if(hx<0.055) color=mix(color,vec3(60,116,98)/255.0,0.18);
        }
    }
    color*=iSceneGain;
