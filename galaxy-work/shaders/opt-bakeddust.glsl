...
void renderOperaScene(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // the sky; drawing them afterwards instead painted stars over the planets.
    {
        ...
        // The dust field is static in screen space, so it can ship as a baked plate: when the channel is
    // bound (its resolution says it is more than the 1x1 placeholder) one fetch replaces a hash, a
    // cell lookup and a twinkle per pixel. With no plate the procedural field still runs, so the
    // shader keeps working on its own.
    if(iChannelResolution[0].x>1.0 && iChannelResolution[0].y>1.0)
        color=max(color, texture(iChannel0, fragCoord/iResolution.xy).rgb*max(iStarGain,0.4));
    else
