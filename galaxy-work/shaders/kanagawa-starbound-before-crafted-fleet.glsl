...
vec3 shipEvent(vec2 fragCoord, vec3 bg) {
    ...
    for(int j=0;j<18;j++) {
        ...
        float emergence=mix(0.10,1.0,easeRelease);
        float impulse=sin(release*3.14159265);
        float lengthScale=(emergence+0.55*impulse)*max(0.025,1.0-pow(depart,3.0));
        float widthScale=mix(0.08,1.0,easeRelease)*max(0.02,1.0-pow(depart,2.0));
