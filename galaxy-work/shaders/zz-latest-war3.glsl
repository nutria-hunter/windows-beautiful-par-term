...
vec3 warPass(vec2 fragCoord, vec2 center, float planetR, vec3 color) {
    ...
    float seed=hash21(vec2(floor(clock/240.0),911.0));
    float age=(mod(clock,240.0)-12.0)/iWarpRate;
    if(age<0.0 || age>56.0) return color;
    float lane=clamp(center.y,0.15,0.85);
    float travel=clamp((age-2.0)/44.0,0.0,1.0);
