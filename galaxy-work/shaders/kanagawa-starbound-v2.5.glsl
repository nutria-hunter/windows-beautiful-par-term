...
// Closed scenic orbits extend beyond the viewport; no on-screen teleport/reset. A lane gives each
// body its own amplitude, vertical centre and height, so they do not all sweep one path.
vec2 orbitLane(float aspect, float phase, float rate, float amp, float mid, float height) {
    float a=phase+iTime*iOrbitSpeed*TAU/1080.0*rate;
    return vec2(aspect*(0.5+amp*cos(a)), mid+height*sin(a));
}
vec2 orbit(float aspect, float phase, float rate, float height) {
    return orbitLane(aspect, phase, rate, 0.78, 0.5, height);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec2 center = orbitLane(aspect,1.2,1.0,0.78,0.50,0.25);   // the hero world, crosses the middle
    vec2 ochre = orbitLane(aspect,1.95,0.73,0.86,0.72,0.20);   // lower lane
    vec2 gas = orbitLane(aspect,-0.24,0.87,0.62,0.30,0.16);    // upper lane
    vec2 ice = orbitLane(aspect,0.55,1.35,0.55,0.14,0.10);     // high and quicker
    vec2 lava = orbitLane(aspect,2.60,0.55,0.70,0.86,0.10);    // slowly along the bottom edge
    // A world far away, on a small slow lane of its own: drawn first and hazed, so every other body
    // crosses in front of it.
    vec2 farBody = orbitLane(aspect,1.75,0.14,0.30,0.60,0.08);
