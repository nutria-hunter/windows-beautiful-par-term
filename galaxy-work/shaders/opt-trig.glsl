...
vec3 hullImpact(vec2 q, float age, vec3 bg) {
    ...
    // Five shards fan out a fixed angle apart. Rotating a unit vector by a constant matrix costs
    // four multiplies, where cos/sin per shard costs ten transcendentals at a quarter rate inside
    // every ship's hit path.
    const vec2 SHARD_STEP=vec2(-0.73736670,0.67549268);
    vec2 v=vec2(1.0,0.0);
    for(int j=0;j<5;j++) {
        vec2 r=q-v*(2.0+age*(28.0+float(j)*5.0));
        if(abs(dot(r,v))<1.4 && abs(dot(r,vec2(-v.y,v.x)))<0.65)
            bg=max(bg,vec3(153,103,69)/255.0*fade);
        v=vec2(v.x*SHARD_STEP.x-v.y*SHARD_STEP.y, v.x*SHARD_STEP.y+v.y*SHARD_STEP.x);
    }
    return bg;
}
