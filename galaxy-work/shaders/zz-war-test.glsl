...
vec3 warPass(vec2 fragCoord, vec2 center, float planetR, vec3 color) {
    if(iWarpRate<=0.0) return color;
    float px=max(floor(iPixelSize),1.0);
    float aspect=iResolution.x/iResolution.y;
    float clock=iTime*iWarpRate;
    float seed=max(hash21(vec2(floor(clock/240.0),911.0)),0.9);
    float age=(mod(clock,40.0)-0.5)/iWarpRate;
    if(age<0.0 || age>36.0) return color;
    float lane=clamp(center.y,0.15,0.85);
    float travel=clamp(age/28.0,0.0,1.0);
    travel=travel*travel*(3.0-2.0*travel);
    vec2 entry=vec2(-0.30,lane), exitP=vec2(aspect+0.30,lane);
    vec2 fwd=normalize(exitP-entry), side=vec2(-fwd.y,fwd.x);
    vec2 base=mix(entry,exitP,travel)*iResolution.y;
    for(int i=0;i<6;i++) {
        float id=float(i);
        vec2 p=(base - fwd*id*0.045*iResolution.y + side*(hash21(vec2(seed,id+3.0))-0.5)*0.09*iResolution.y)/px;
        vec2 d=fragCoord/px-p;
        float u=dot(d,fwd), v=dot(d,side);
        float half=2.4-0.30*clamp(u+2.4,0.0,4.8);
        if(abs(u)>4.6 || abs(v)>half) continue;
        color=max(color,vec3(32,38,48)/255.0);
        if(abs(v)<half*0.45) color=max(color,vec3(56,64,76)/255.0);
        if(u<-1.4 && u>-3.0 && abs(v)<1.0) color=max(color,vec3(60,180,196)/255.0);
    }
    vec2 cB=floor(center*iResolution.y)/px;
    for(int m=0;m<3;m++) {
        vec2 st=cB+stationPoint(0.65+float(m)*TAU/3.0)*planetR*(iResolution.y/px);
        for(int i=0;i<6;i++) {
            float id=float(i);
            float phase=mod(age-float(m)*0.4-float(i)*0.11,3.2);
            if(phase>0.22) continue;
            vec2 tgt=(base - fwd*id*0.045*iResolution.y)/px;
            vec2 toward=normalize(tgt-st);
            vec2 rel=fragCoord-mix(st,tgt,phase/0.22)*px;
            float along=dot(rel,toward);
            if(abs(along)<4.0 && length(rel-toward*along)<0.7) color=max(color,vec3(186,244,252)/255.0);
            if(phase<0.07 && abs(fragCoord.x-st.x*px)<2.0 && abs(fragCoord.y-st.y*px)<2.0)
...
void renderOperaScene(out vec4 fragColor, in vec2 fragCoord) {
    ...
    color=warPass(fragCoord,center,CITY_R,color);
