...
vec3 cityPlanet(vec2 q, vec2 center) {
    ...
    // Broken city belts and districts: lights exist only on sparse building lots.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Integer-snapped centers prevent fractional-pixel sprites during travel.
    center=floor(center*size.y)/size.y;
    ochre=floor(ochre*size.y)/size.y;
    gas=floor(gas*size.y)/size.y;
    moon=floor(moon*size.y)/size.y;
    vec2 q = (p-center)/0.205;
    vec2 gq=(p-gas)/0.15;
    if(gq.y<0.27*gq.x) color=ring(gq,color);
    color=planet(p,gas,0.15,0.0,color);
    if(gq.y>=0.27*gq.x) color=ring(gq,color);
    color=planet(p,ochre,0.074,1.0,color);
    color=planet(p,moon,0.025,0.0,color);
    ...
    bool empty = length(q)>1.75 && length(gq)>1.95 && distance(p,ochre)>0.08
        && distance(p,moon)>0.03;
