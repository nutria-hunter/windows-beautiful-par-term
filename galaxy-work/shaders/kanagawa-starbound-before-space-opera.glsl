...
vec3 alienGate(vec2 q, float phase, float depth, vec3 bg) {
    return metricWake(q,clamp(phase*6.0,0.0,1.0),depth,bg,true);
}
...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, float railShieldStr, vec3 color) {
    ...
    for(int j=0;j<9;j++) {
        ...
        if(abs(s2.x)<60.0 && abs(s2.y)<40.0)
            color=damagedHull(s2,kind,age+id,0.34,color,armorLight(center/size.y,forward),true,raiderDownTime(id,seed)-age);
    ...
    if(defended && dCount>0) {
        ...
        for(int k=0;k<5;k++) {
            ...
            if(abs(gd.x)<gspan && abs(gd.y)<gspan) {
                ...
                float shortPhase=age<life*0.5 ? clamp(dIn*8.0,0.0,1.0) : clamp(dOut*6.0,0.0,1.0);
            ...
            if(age<down) {
                ...
                if(abs(s3.x)<48.0 && abs(s3.y)<26.0)
                    color=damagedHull(s3,kind,age+id,0.36,color,armorLight(dcenter/size.y,dForward),false,down-age);
...
vec3 shipEvent(vec2 fragCoord, vec3 bg) {
    ...
    for(int j=0;j<18;j++) {
        ...
        if(t<=0.0 || t>=cruiseEnd+0.375) continue;
        ...
        float arrive=clamp(t/0.425,0.0,1.0);
        float depart=clamp((t-cruiseEnd)/0.375,0.0,1.0);
        ...
        if(abs(gateDelta.x)<gateSpan && abs(gateDelta.y)<gateSpan) {
            ...
            float gatePhase=t<0.425 ? arrive : depart;
        ...
        color=warpTrail(q,t<0.425 ? arrive : depart,depth,color);
