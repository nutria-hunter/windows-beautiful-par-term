...
vec3 asteroidGroup(vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int j=0;j<26;j++) {
        ...
        if(hash21(floor(d)+vec2(id*11.0,id*5.0)) > 0.88) tone = vec3(36,38,46)/255.0;  // craters
...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, float railShieldStr, vec3 color) {
    ...
    for(int j=0;j<9;j++) {
        ...
        for(int b=0;b<3;b++) {
            ...
            vec2 dirB=normalize(hitPt-center);
            if(bAge<0.42) {
                vec2 ppos=mix(center,hitPt,bAge/0.42);
                vec2 rel=pixel-ppos;
                float along=dot(rel,dirB);
                if(abs(along)<9.0 && length(rel-dirB*along)<0.8)
                    color=max(color,vec3(240,150,152)/255.0);
            } else {
                float fade=(bAge-0.42)/0.28;
                vec2 rel=pixel-hitPt;
                if(((railHit ? railShieldStr : shieldStr)>0.02) && !duelling) {
                    // The shield answers as a green hexagon at the point of contact.
                    float hr=4.0-2.6*fade;
                    if(hr>0.8 && hexEdge(rel,hr)<1.1) color=max(color,vec3(152,216,180)/255.0);
                    if(fade<0.30 && hexEdge(rel,hr)<2.8)
                        color=max(color,mix(color,vec3(52,102,86)/255.0,0.55));
                } else {
                    // Small, hard impact: a two-pixel flash and a thin ring.
                    float w=1.8-fade*2.1;
                    if(w>0.4 && abs(rel.x)<=w && abs(rel.y)<=w)
                        color=max(color,vec3(246,208,196)/255.0);
                    if(w>0.4 && abs(max(abs(rel.x),abs(rel.y))-(w+1.0))<0.9 && fade<0.6)
    ...
    if(attack>0.5) {
        for(int m=0;m<3;m++) {
            ...
            if(bAge>0.0 && bAge<0.45) {
                ...
                if(abs(along)<7.0 && length(rel-toward*along)<0.8)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(abs(q.x)<1.8 && abs(q.y)<1.5) {
        ...
        // The ring's own shield: an elongated hexagon hugging the rail, in the same green, on its own
        // breaking cycle.
        if(shieldRail.x>0.02) {
            vec2 ef=vec2(q.x+0.30*q.y,q.y-0.30*q.x)/1.09;
            float hx=hexEdge(vec2(ef.x/1.62,ef.y/0.52),1.0);
            if(hx<0.030) color=max(color,vec3(146,208,176)/255.0*shieldRail.x);
            else if(hx<0.075) color=max(color,mix(color,vec3(48,96,82)/255.0,0.40));
        }
    }
    color*=iSceneGain;
