...
// The orbital ring's batteries. Extracted from `incursionEvent` unchanged in behaviour: the shader
...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, float railShieldStr, vec3 color) {
    ...
    color=ringBarrage(color,fragCoord,px,size,target,targetR,age,seed,float(count),formation,forward,sidev);
