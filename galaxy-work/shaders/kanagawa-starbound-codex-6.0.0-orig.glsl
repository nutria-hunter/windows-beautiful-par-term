...
vec3 starDust(vec2 fragCoord, vec3 color) {
    ...
    return max(color, col*(0.18+0.30*tw)*iStarGain);
}
...
vec3 patrolPass(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int k=0;k<3;k++) {
        ...
        vec2 d=chunkLocal(fragCoord,p2p,px);
        ...
        color=warshipArt(d/dep,kind,age*0.5+id,dep,color,armorLight(p2p/size.y,vec2(1,0)),false);
    }
    return color;
}
...
vec3 structurePass(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    if(hash21(vec2(seed,31.0))<0.5) {
        ...
        vec3 dark=vec3(18,23,33)/255.0, mid=vec3(36,45,57)/255.0, lit=vec3(57,69,81)/255.0;
        vec3 bright=vec3(76,85,92)/255.0, win=vec3(114,104,77)/255.0;
        ...
        if(abs(d.x)<r*0.16 && abs(d.y)<r*1.05) {
            ...
            if(d.y>r*0.20 && d.y<r*0.40) tone=dark;
...
// DARK ART CONTRACT: three armor masses, one recessed bay, rare functional lights.
// Coordinates/motion remain owned by placeAt; all hulls share the same materials.
vec2 armorLight(vec2 worldCenter, vec2 forward) {
    vec2 lamp=vec2(iResolution.x/iResolution.y*0.32,-0.35);
...
vec3 warshipArt(vec2 s, float kind, float age, float depth, vec3 bg, vec2 light, bool alien) {
    ...
    float u=(s.x-aft)/(bow-aft);
    float ay=abs(s.y);
    // Faceted aft corners and a real taper, never OR'ed with a covering rectangle.
    float width=beam*min(1.0,0.72+u*2.3);
    float shoulder=kind>1.5 && kind<2.5 ? 0.76 : 0.30;
    width*=1.0-clamp((u-shoulder)/(1.0-shoulder),0.0,1.0)*(kind>4.5 ? 0.35 : 0.94);
    bool body=u>=0.0 && u<=1.0 && ay<width;
    if(alien) {
        // Layered carapaces around a long dark cleft. Shape, not neon, marks origin.
        float lobe=beam*(0.33+0.64*sin(clamp(u,0.0,1.0)*3.14159265));
        float arc=(kind<1.5 ? 0.28 : -0.22)*beam*sin(u*3.14159265);
        body=u>=0.0 && u<=1.0 && abs(s.y-arc)<lobe;
        if(u>0.42 && u<0.92 && abs(s.y-arc)<beam*0.16) body=false;
        if(kind>2.5 && s.y>beam*0.45 && u>0.2 && u<0.5) body=false;
    }
    vec3 c=bg;
    if(body) {
        vec3 shadow=vec3(18,23,33)/255.0;
        vec3 plate=vec3(46,57,69)/255.0;
        vec3 lit=vec3(76,85,92)/255.0;
        if(alien) {plate=vec3(48,47,60)/255.0;lit=vec3(77,75,87)/255.0;}
        float face=sign(s.y)*light.y;
        vec3 material=face>0.25 ? plate : shadow;
        // Large upper deck: offset polygon, not a glowing full-length center stripe.
        bool deck=u>0.12 && u<0.78 && ay<width*0.62;
        if(deck) material=face>-0.25 ? lit : plate;
        if(ay>width-0.65 && face>0.4 && u>0.10 && u<0.65) material=lit;
        if(alien && u>0.38 && u<0.43) material=shadow;
        // One recessed machinery trench and one bridge block, never a periodic grid.
        bool detailed=depth>0.30;
        if(detailed && u>0.19 && u<0.55 && s.y>width*0.22 && s.y<width*0.43)
            material=shadow;
        if(detailed && u>0.19 && u<0.32 && s.y>-width*0.36 && s.y<width*0.12)
            material=plate;
        if(detailed && u>0.19 && u<0.25 && s.y>-width*0.36 && s.y<-width*0.20)
            material=lit;
        // Carrier hangar is a single long dark bay; transports have just three loads.
        if(kind>1.5 && kind<2.5 && u>0.28 && u<0.73 && s.y>width*0.10 && s.y<width*0.56)
            material=shadow*0.70;
        if(kind>4.5 && u>0.24 && u<0.76 && ay<width*0.65) {
            float bay=fract((u-0.24)*5.75);
            material=bay<0.13 ? shadow : plate;
        }
        if(detailed && u>0.43 && u<0.56 && s.y>width*0.32 && s.y<width*0.39)
            material=vec3(114,104,77)/255.0;
        c=material*iShipGain*(0.76+0.24*clamp(depth,0.0,1.0));
    }
    // One or two compact engine recesses. Functional illumination, not a neon outline.
    float engineY=beam*0.36;
    float reach=1.8+0.45*sin(age*1.2);
    if(s.x<aft+0.8 && s.x>aft-reach && (abs(s.y-engineY)<1.1 || abs(s.y+engineY)<1.1))
        c=max(c,vec3(74,120,129)/255.0*iShipGain*(0.65+0.35*depth));
    return c;
}
...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, float railShieldStr, vec3 color) {
    ...
    for(int j=0;j<9;j++) {
        ...
        if(abs(s2.x)<60.0 && abs(s2.y)<40.0)
            color=warshipArt(s2,kind,age+id,0.34,color,armorLight(center/size.y,forward),true);
        ...
        for(int k=0;k<5;k++) {
            ...
            if(age<down) {
                ...
                if(abs(s3.x)<48.0 && abs(s3.y)<26.0)
                    color=warshipArt(s3,kind,age+id,0.36,color,armorLight(dcenter/size.y,dForward),false);
...
vec3 shipEvent(vec2 fragCoord, vec3 bg) {
    ...
    for(int j=0;j<18;j++) {
        ...
        if(abs(s.x)<58.0 && abs(s.y)<30.0)
            color=warshipArt(s,kind,t+rnd,depth,color,armorLight(center/iResolution.y,forward),false);
    }
    return color;
}
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float k = floor(t+0.12); // Broad ink masses; no screen-wide ordered dither.
    vec3 color = ink(k);
    ...
    if(k>=2.0 && hash21(pixel+7.0)>0.9997)
    ...
    // the sky; drawing them afterwards instead painted stars over the planets.
    {
        ...
        color=max(color,stars(floor(fragCoord),29.0*px,0.0,1.0,starCloud)+stars(floor(fragCoord),47.0*px,1.0,1.0,starCloud)
            +stars(floor(fragCoord),71.0*px,2.2,2.0,starCloud)+stars(floor(fragCoord),17.0*px,1.6,1.0,starCloud));
