...
vec3 stars(vec2 pixel, float cellSize, float driftRate, float boost, float cloud) {
    ...
    vec3 color = vec3(214,224,236)/255.0;              
    if(hue>0.86) color=vec3(190,206,238)/255.0;        
    else if(hue>0.72) color=vec3(226,206,164)/255.0;   
    else if(hue>0.62) color=vec3(232,190,186)/255.0;   
    else if(hue>0.54) color=vec3(196,180,226)/255.0;   
    else if(hue>0.48) color=vec3(174,214,208)/255.0;   
...
vec3 structurePass(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    float r=(0.022+0.014*hash21(vec2(seed,23.0)))*size.y;   
        vec2 d=placeAt(fragCoord,center,px,0.5);   
...
vec3 asteroidGroup(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int j=0;j<26;j++) {
        ...
        float wr = r3>0.86 ? 6.5 : (r3>0.55 ? 4.2 : 2.6);   
        ...
        if(dd > (rr-1.8)*(rr-1.8) && g < 0.0) tone = vec3(64,65,69)/255.0;   
        ...
        if(mined) {
            ...
            for(int k=0;k<3;k++) {
                ...
                if(abs(dq.x)<1.8 && abs(dq.y)<1.4) {
                    ...
                    if(dq.x>0.6) color=max(color,vec3(81,117,129)/255.0);   
                }
...
vec2 shieldState(float attackAge, float seed) {
    ...
    if(u<0.9) return vec2(u/0.9,0.0);                          
    if(u<0.9+upTime) return vec2(1.0-0.55*(u-0.9)/upTime,0.0);  
    float since=u-(0.9+upTime);
    return vec2(0.0, since<0.30 ? 1.0-since/0.30 : 0.0);       
}
...
void renderOperaScene(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec2 center = orbitLane(aspect,1.2,1.0,0.78,0.50,1.0);       
    vec2 gas = orbitLane(aspect,4.10,0.87,0.62,0.30,3.2);        
    vec2 ochre = orbitLane(aspect,2.30,0.73,0.86,0.72,3.8);      
    vec2 ice = orbitLane(aspect,5.40,1.35,0.55,0.14,4.4);        
    vec2 lava = orbitLane(aspect,0.60,0.55,0.70,0.86,5.0);       
