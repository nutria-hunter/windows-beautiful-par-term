...
// The planets' rule, at screen resolution: anchor the origin to the physical pixel grid, so the offsets
// step one pixel at a time and the shape's raster never re-boils. The planets get away with sampling on
// the 4px block grid because they are slow and huge; a small fast object sampled that way sizzles, so
// these use the fragment itself.
vec2 fineLocal(vec2 fragCoord, vec2 center, float px) {
    return (fragCoord-floor(center*px))/px;
}
// ── THE ONLY PLACEMENT ROUTINE ──────────────────────────────────────────────────────────────
// Every moving object in this mural - planet, ship, rock, comet, gate - is placed through `placeAt`.
// The origin is snapped to the physical pixel grid (motion is one pixel at a time, exactly like the
// planets) and the local lattice is quantised to `step` logical pixels (the art keeps the scene's chunky
// look). Sampling happens at the *fragment*: sampling the block centre instead makes every sub-pixel move
// flip a whole boundary block, which is the shimmer this function exists to prevent. A new object must
// call this and nothing else - `galaxy-work/work/check_placement.py` fails when a raw `pixel - ...`
// offset reappears, so the rule is enforced by the toolchain rather than by memory.
vec2 placeAt(vec2 fragCoord, vec2 center, float px, float step) {
    return floor(fineLocal(fragCoord,center,px)/step)*step;
}
// Ships sit one step in from the planets: 2 screen px of detail (0.5 logical px), which divides the 4px
// block exactly, so the plating reads without the object drifting off the scene grid.
vec2 chunkLocal(vec2 fragCoord, vec2 center, float px) {
    return placeAt(fragCoord,center,px,0.5);
}
// ── RESOLUTION AND LIGHTING PRINCIPLES (structural, not remembered) ──────────────────────────
// 1) Resolution follows *apparent size*, never habit. A planet is ~400 screen px across and wants the
//    mural's 4px chunk; a ship is ~40 and wants 2px; a distant speck is ~8 and should be drawn at 1px,
//    because forcing it to 4px blocks is what made far objects mushy. Every object asks `lodStep` for
//    its lattice instead of choosing one, so "무지성 저해상도" cannot come back through a new feature.
float lodStep(float apparentPx) {
    if(apparentPx > 64.0) return 4.0;
    return 2.0;
}
// 2) Objects do not invent their own lighting. `shade3` gives three flat tones from the same light
//    direction the planets use, so a new object is automatically lit the way the rest of the mural is.
//    (Third tone + a lit edge is the minimum that reads as a *thing* rather than a blob.)
...
vec3 infrastructure(vec2 q, vec3 bg, float front, float pixelWidth) {
    ...
    if(frontHalf == (front>0.5)) {
        ...
        // Rail: a thicker band with a lit upper edge, so the orbit reads as built infrastructure.
        if(abs(rail-1.0)<pixelWidth*2.2) c=vec3(43,67,79)/255.0;
        if(abs(rail-1.0)<pixelWidth*0.9) c=vec3(68,96,112)/255.0;
    }
    for(int j=0;j<3;j++) {
        ...
        if(segmentDistance(q,foot,anchor)<pixelWidth*1.1) c=vec3(54,72,85)/255.0;
        // Cable dashes, so the elevator line reads as a drawn thread rather than a solid smear.
        if(segmentDistance(q,foot,anchor)<pixelWidth*1.1 && mod(floor(length(anchor-foot)*40.0),3.0)<2.0)
            c=vec3(74,98,114)/255.0;
        vec2 d=abs(q-anchor);
        // Station: a hub with arms and lit ports.
        if(d.x<0.085 && d.y<0.045) c=vec3(65,91,107)/255.0;
        if(d.x<0.055 && d.y<0.028) c=vec3(88,116,132)/255.0;
        if(mod(floor(q.x*90.0),4.0)<1.0 && d.x<0.085 && d.y<0.045) c=vec3(198,176,120)/255.0;
...
vec3 patrolPass(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    for(int k=0;k<3;k++) {
        ...
        vec2 d=placeAt(fragCoord,p2p,px,1.0);
        ...
        color=fleetHull(d/dep,kind,age*0.5+id,dep,color);
    }
    return color;
}
...
vec3 structurePass(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    if(hash21(vec2(seed,31.0))<0.5) {
        // Stargate, drawn as part of the mural rather than as a machine: two quiet ink rings with a lit
        // upper-left edge like every other body, a dithered ink vortex in the mouth, thin radial streaks
        // like the nebula's contour ribbons, three tapered cut-paper pylons and two slow motes. No bright
        // trim and no chevrons - the palette and the silhouette do the work.
        // Seen from the side: a ring standing at an angle, so it reads as a gate the scene drifts past
        // rather than a face-on hoop staring at the viewer. The mouth is the same ellipse, squashed to a
        // slit. This is the one shape in the mural that should never be seen straight on.
        vec2 de=vec2(d.x/0.30,d.y);
        float rr=length(de);
        float ang=atan(d.y,d.x);
        vec3 ink1=vec3(38,44,58)/255.0, ink2=vec3(58,72,92)/255.0, ink3=vec3(84,104,128)/255.0;
        if(abs(rr-r)<2.0) color=max(color, d.x*0.7+d.y*1.2 < 0.0 ? ink3 : ink1);
        if(abs(rr-r*0.88)<1.1) color=max(color,ink2);
        // Mouth: concentric ink bands, drawn the way a planet's cloud belts are - a continuous function
        // of the radius, so nothing re-rasterises as the gate drifts.
        if(rr<r*0.94) {
            float band=0.5+0.5*sin(rr/max(r,1.0)*7.0-iTime*0.25+seed*6.0);
            float li=ditherLevel(0.10+0.30*band,pixel,4.0);
            color=max(color,mix(vec3(22,26,38)/255.0,vec3(84,108,138)/255.0,li));
        }
        if(abs(rr-r)<1.2) color=max(color, d.x*0.7+d.y*1.2 < 0.0 ? ink3 : vec3(30,35,48)/255.0);
        // Radial streaks, thin as brush strokes.
        if(rr<r*0.90 && rr>r*0.20 && mod(ang*3.82+seed*5.0,1.0)<0.05)
            color=max(color,vec3(96,120,148)/255.0);
        // Three pylons: flat two-tone shapes, no glow.
        for(int k=0;k<3;k++) {
            float a=float(k)*2.0943951+0.7;
            vec2 pd=d-vec2(cos(a),sin(a))*(r+4.0);
            float along=pd.x*cos(a)+pd.y*sin(a);
            float across=-pd.x*sin(a)+pd.y*cos(a);
            if(abs(across)<2.4 && along>-6.0 && along<4.0)
                color=max(color, across<0.0 ? ink2 : ink1);
        }
        // Two slow motes inside the mouth, like the rail's vehicles.
        for(int k=0;k<2;k++) {
            float a2=iTime*0.22*(1.0+0.3*float(k))+float(k)*3.1+seed*7.0;
            vec2 mp=d-vec2(cos(a2),sin(a2))*r*0.55;
            if(abs(mp.x)<1.2 && abs(mp.y)<1.2) color=max(color,vec3(150,178,200)/255.0);
        }
    } else {
        ...
        vec3 dark=vec3(38,42,50)/255.0, mid=vec3(62,68,78)/255.0, lit=vec3(92,98,108)/255.0;
        vec3 bright=vec3(126,132,142)/255.0, win=vec3(176,158,110)/255.0;
        float glow=abs(d.x*0.7+d.y*1.2);                    // light from the upper left
        // One light direction for every part, through the shared helper - see `shade3`.
        float sLit=-(d.x*0.7+d.y*1.2);
        // Spine: segment bands, brighter on the lit side.
        if(abs(d.x)<r*0.16 && abs(d.y)<r*1.05) {
            vec3 tone=shade3(sLit, lit, bright, mid);
        ...
        if(ringD<r*0.14) {
            ...
            // Windows as discrete dots, not an angular pattern: a per-pixel angle shifts under the station
            // as it drifts, which is what made it look like it was boiling.
            for(int k=0;k<12;k++) {
                float a2=float(k)*0.5235988+iTime*0.35+seed*9.0;
                vec2 wp=d-vec2(cos(a2),sin(a2))*(r*0.62);
                if(abs(wp.x)<r*0.07 && abs(wp.y)<r*0.07) color=max(color,win);
            }
        }
        ...
        for(int w=0;w<2;w++) {
            ...
            if(abs(q.x)<r*0.08 && abs(q.y)<r*0.32) color=max(color,mid);
            float px1=r*1.55, py=r*0.31;
            if(abs(q.x)>r*0.30 && abs(q.x)<px1 && abs(q.y)<py) {
                // A flat panel with a lit leading edge - no checker, so it cannot shimmer.
                color=max(color, q.x<0.0 ? vec3(32,42,61)/255.0 : vec3(45,61,78)/255.0);
                if(abs(q.y)>py-r*0.08) color=max(color,lit);
            }
        }
        ...
        for(int w=0;w<2;w++) {
            ...
            if(abs(q.y)<r*0.08 && abs(q.x)<r*0.42) color=max(color,mid);
            if(abs(q.x-r*0.48)<r*0.20 && abs(q.y)<r*0.20) {
                color=max(color, glow<2.0 ? lit : mid);
                if(abs(q.x-r*0.48)<r*0.09 && abs(q.y)<r*0.09) color=max(color,win);
            }
            if(abs(q.x-r*0.56)<r*0.07 && abs(q.y)<r*0.07)
                color=max(color, w==0 ? vec3(195,64,67)/255.0 : vec3(106,149,137)/255.0);
        }
        // Beacon mast, with a slow blink at the tip.
        if(abs(d.x)<r*0.06 && d.y<-r*1.0 && d.y>-r*1.45) color=max(color,mid);
        if(length(d-vec2(0.0,-r*1.48))<r*0.10 && 0.5+0.5*sin(iTime*1.6+seed*10.0)>0.45)
            color=max(color,vec3(226,120,110)/255.0);
        // Lit windows in the spine's unbanded segments.
        if(abs(d.x)<r*0.16 && abs(d.y)<r*1.0 && mod(floor(d.y/3.5),2.0)>=1.0
...
vec3 asteroidGroup(vec2 fragCoord, vec2 pixel, vec2 size, vec3 color) {
    ...
    vec2 endp=vec2(aspect+0.62, lane+(hash21(vec2(seed,23.0))-0.5)*0.30);
    ...
    for(int j=0;j<26;j++) {
        ...
        vec3 tone = vec3(72,74,78)/255.0;
        if(g < -wr*0.30) tone = vec3(96,98,102)/255.0;
        else if(g > wr*0.35) tone = vec3(44,46,52)/255.0;
        if(dd > (rr-1.8)*(rr-1.8) && g < 0.0) tone = vec3(116,118,122)/255.0;   // rim light
        ...
        bool mined = false;
        vec3 ore=vec3(198,168,104)/255.0;
        float oreRoll=hash21(vec2(seed*59.0,id+29.0));
        if(oreRoll<0.34) ore=vec3(196,202,210)/255.0;
        else if(oreRoll<0.62) ore=vec3(126,156,216)/255.0;
        if(mined) {
        ...
        if(mined) {
            ...
            for(int k=0;k<3;k++) {
                ...
                if(abs(dq.x)<1.8 && abs(dq.y)<1.4) {
                    color=max(color,vec3(118,124,134)/255.0);
                    if(dq.x>0.6) color=max(color,vec3(150,220,240)/255.0);   // engine
                }
...
vec3 alienHull(vec2 s, float kind, float age, float depth, vec3 bg) {
    float sy=abs(s.y);
    bool hull=false;
    // One design language for the whole squadron: long, low darts on a single axis, a straight spine and one
    // hard step. Serious silhouettes read at a glance and survive being drawn very small - which is the size
    // they are actually shown at next to a planet.
    if(kind<0.5) {
        // Dagger: a slender arrowhead.
        hull=(s.x>=-22.0 && s.x<=24.0 && sy<=max(1.0,(24.0-s.x)*0.16))
            || (s.x>=-26.0 && s.x<=-14.0 && sy<=3.0);
    } else if(kind<1.5) {
        // Wedge with side pods.
        hull=(s.x>=-26.0 && s.x<=22.0 && sy<=max(1.2,(22.0-s.x)*0.22))
            || (s.x>=-20.0 && s.x<=-2.0 && sy>4.0 && sy<9.0);
    } else if(kind<2.5) {
        // Spire: a narrow vertical dart.
        hull=(s.x>=-16.0 && s.x<=16.0 && sy<=max(1.2,(16.0-s.x)*0.30))
            || (s.x>=-12.0 && s.x<=4.0 && sy<10.0);
    } else if(kind<3.5) {
        // Long wedge, the flagship: a wide blade with a bridge step.
        hull=(s.x>=-34.0 && s.x<=34.0 && sy<=max(1.2,(34.0-s.x)*0.26))
            || (s.x>-22.0 && s.x<-6.0 && sy>4.0 && sy<9.0);
    } else {
        // Carrier: a flat slab with a hangar notch and a blunt bow.
        hull=(s.x>=-30.0 && s.x<=26.0 && sy<=max(2.0,(26.0-s.x)*0.30))
            || (s.x>=-18.0 && s.x<8.0 && sy>5.0 && sy<10.0);
    }
    vec3 c=bg;
    if(hull) {
        // Dark gunmetal hull with muted steel highlights: the squadron should read as warships at dusk, not as
        // bright toys. Only the markings carry colour, and even those are dimmed a step.
        vec3 shell=vec3(34,30,42)/255.0;
        if(s.y<0.0) shell=vec3(74,70,84)/255.0;
        if(sy<2.0) shell=vec3(104,100,114)/255.0;
        if(mod(floor(s.x)+30.0,7.0)<1.0 && sy>2.0) shell=vec3(26,22,34)/255.0;
        // Crimson sensor band and gun ports: Kanagawa's own red, so the raiders belong to the same palette
        // as everything else on screen.
        // Each hull class carries its own accent from the Kanagawa palette, so a raid does not read as one
        // monotonous pink mass: crimson, burnt orange, spring violet, boat yellow, crystal blue.
        vec3 accent=vec3(148,52,58)/255.0;
        if(kind>0.5 && kind<1.5) accent=vec3(142,86,56)/255.0;
        else if(kind>1.5 && kind<2.5) accent=vec3(116,100,132)/255.0;
        else if(kind>2.5 && kind<3.5) accent=vec3(134,104,58)/255.0;
        else if(kind>3.5) accent=vec3(96,120,164)/255.0;
        if(s.x>-6.0 && s.x<10.0 && sy<2.5 && mod(floor(s.x),3.0)<1.0)
            shell=accent;
        if(kind>3.5 && sy>11.0 && sy<17.0 && mod(floor(s.x)+14.0,5.0)<2.0)
            shell=mix(accent,vec3(150,148,156)/255.0,0.35);
        // Per-class fittings, so silhouette and colour both change between kinds.
        if(kind>2.5 && kind<3.5 && s.x>10.0 && s.x<24.0 && sy<3.0) shell=accent;      // wedge hangar bay
        else if(kind<0.5 && sy>3.0 && sy<5.0 && mod(floor(s.x)+40.0,6.0)<2.0) shell=accent;  // scythe wing slots
        else if(kind>0.5 && kind<1.5 && s.x>16.0 && sy>3.0 && sy<9.0) shell=accent;   // crab claw tips
        // Plating, the same single light direction as everything else: bright top edge, shaded waist,
        // turret dots, and the spine kept apart from the wings.
        if(sy<1.0) shell=vec3(112,108,122)/255.0;
        else if(s.y>0.0 && sy>2.0) shell=mix(shell,vec3(24,20,32)/255.0,0.55);
        if(sy>1.2 && sy<2.6 && mod(floor(s.x)+40.0,5.0)<1.0) shell=vec3(128,124,136)/255.0;
        if(mod(floor(s.x)+22.0,11.0)<1.0 && sy>3.0) shell=vec3(30,26,38)/255.0;
    ...
    if(s.x<engineX && s.x>engineX-flame*1.6 && (abs(sy-2.0)<1.4 || abs(sy-6.0)<1.4))
        c=max(c,vec3(140,86,56)/255.0*depth);
    return c;
}
...
vec3 alienGate(vec2 q, float phase, float depth, vec3 bg) {
    ...
    vec3 c=bg;
    if(r<0.82) c*=0.35;
    if(abs(r-1.0)<0.075) c=max(c,vec3(126,46,62)/255.0*opening);
    if(abs(r-1.26)<0.04) c=max(c,vec3(94,44,110)/255.0*opening);
    if(abs(q.y)<1.0 && abs(q.x)<(11.0+36.0*opening))
        c=max(c,vec3(138,58,70)/255.0*opening);
    return c;
}
...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, float railShieldStr, vec3 color) {
    ...
    for(int j=0;j<9;j++) {
        ...
        vec2 p2=chunkLocal(fragCoord,center,px);
    ...
    if(attack>0.5) {
        for(int m=0;m<3;m++) {
            ...
            vec2 stLocal=vec2(e.x-0.30*e.y,e.y+0.30*e.x);   // inverse of infrastructure's frame (no extra scale)
    ...
    if(defended && dCount>0) {
        ...
        for(int k=0;k<5;k++) {
            ...
            if(age<down) {
                vec2 p3=chunkLocal(fragCoord,dcenter,px);
...
vec2 shipSchedule(float time) {
    if(iWarpRate<=0.0) return vec2(-100.0,0.0);
    float clock=time*iWarpRate;
    float slot=floor(clock/340.0);
    float seed=hash21(vec2(slot,81.0));
    if(seed<0.22) return vec2(-100.0,seed);
    float delay=30.0+140.0*hash21(vec2(slot,19.0));
...
vec3 warpGate(vec2 q, float phase, float depth, vec3 bg) {
    ...
    if(abs(r-1.0)<0.07 || abs(r-1.24)<0.035)
        c=max(c,vec3(52,92,110)/255.0*iShipGain*opening);
    if(abs(r-0.86)<0.045)
        c=max(c,vec3(86,74,106)/255.0*iShipGain*opening);
    if(abs(q.y)<1.0 && abs(q.x)<(10.0+35.0*opening)*depth)
        c=max(c,vec3(74,106,122)/255.0*iShipGain*opening);
    return c;
}
vec3 fleetHull(vec2 s, float kind, float age, float depth, vec3 bg) {
    float sy=abs(s.y);
    ...
    if(hull) {
        ...
        // Plating: one light direction, like the planets - a bright top edge, a shaded waist, a bridge
        // block and turret dots. The detail is what makes a hull read as built rather than extruded.
        if(sy<1.0) metal=vec3(112,118,120)/255.0;
        else if(s.y>0.0 && sy>2.0) metal=mix(metal,vec3(30,38,50)/255.0,0.45);
        if(kind<0.5 && s.x>-9.0 && s.x<1.0 && sy<6.0) metal=vec3(104,112,118)/255.0;
        if(kind>1.5 && kind<2.5 && s.x>-10.0 && s.x<4.0 && sy<4.0) metal=vec3(104,112,118)/255.0;
        if(sy>1.2 && sy<2.6 && mod(floor(s.x)+50.0,6.0)<1.0) metal=vec3(128,134,138)/255.0;
        // Navigation lights at the extremities: red to port, teal to starboard.
        if(s.x>27.0 && sy<1.3) metal=vec3(195,64,67)/255.0;
        if(s.x<-23.0 && sy<1.3) metal=vec3(106,149,137)/255.0;
        ...
        // Plating and detail: the same single light direction, a brighter top edge, a shaded waist, and
        // turret dots along the spine. The planets read because every part shares one light; so should
        // every hull.
        if(sy<1.0) metal=vec3(112,118,120)/255.0;
        else if(s.y>0.0 && sy>2.0) metal=mix(metal,vec3(30,38,50)/255.0,0.45);
        if(sy>1.2 && sy<2.6 && mod(floor(s.x)+40.0,5.0)<1.0) metal=vec3(128,134,138)/255.0;
        if(kind>4.5) {
...
vec3 shipEvent(vec2 fragCoord, vec3 bg) {
    ...
    for(int j=0;j<18;j++) {
        ...
        float depth = cls>1.5 ? 0.20+id*0.010 : 0.30+id*0.080;
        float kind;
        if(cls>1.5) {
        ...
        float speed=iResolution.y*1.15/cruiseEnd;
        ...
        vec2 pixel=chunkLocal(fragCoord,center,px);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    color=asteroidGroup(fragCoord,pixel,size,color);
    color=patrolPass(fragCoord,pixel,size,color);
    color=structurePass(fragCoord,pixel,size,color);
