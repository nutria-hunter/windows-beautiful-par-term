...
vec3 asteroidGroup(vec2 pixel, vec2 size, vec3 color) {
    ...
    int count=8+int(floor(hash21(vec2(seed,71.0))*9.0));
    for(int j=0;j<16;j++) {
        if(j>=count) continue;
        float id=float(j);
        ...
        vec2 rock = center - dir*(r1*0.16+progress*0.10)*size.x*0.5 + side*(r2-0.5)*0.24*size.y;
        vec2 d = pixel-rock;
        float s1=hash21(vec2(seed*7.0,id+3.0));
        float w = s1>0.90 ? 2.5 : (s1>0.62 ? 1.5 : 0.9);   // half-extent in logical pixels
        if(abs(d.x)>w || abs(d.y)>w) continue;
        // Three flat tones: a sunlit top, a dark underside, and the body between them.
        vec3 tone=vec3(92,88,80)/255.0;
        if(d.y < -w*0.35) tone=vec3(126,120,108)/255.0;
        else if(d.y > w*0.35) tone=vec3(50,48,46)/255.0;
...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, vec3 color) {
    float aspect=size.x/size.y;
    ...
    for(int j=0;j<6;j++) {
        ...
        vec2 center=formation+sidev*(r1-0.5)*0.11*size.y-forward*id*0.035*size.y;
        ...
        if(attack>0.5) {
            ...
            vec2 limb=tgtPx-toward*targetR*size.y;
            float boltAge=mod(age-id*0.19,1.15);
            if(boltAge<0.50) {
                // Bolt: a long hot segment travelling from the hull to the limb, so the line of fire is
                ...
                // Impact: a hard burst on the limb, shrinking as it fades, with a one-frame ring.
                vec2 rel=pixel-limb;
                float fade=(boltAge-0.50)/0.28;
                float w=5.0-fade*9.0;
                if(w>0.4) {
                    if(abs(rel.x)<=w && abs(rel.y)<=w)
                        color=max(color,vec3(248,206,196)/255.0);
                    float ring=w+1.6;
                    if(abs(max(abs(rel.x),abs(rel.y))-ring)<1.2 && fade<0.5)
                        color=max(color,vec3(226,116,124)/255.0);
                }
            }
        }
    }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    color=incursionEvent(fragCoord,pixel,size,center,CITY_R,color);
