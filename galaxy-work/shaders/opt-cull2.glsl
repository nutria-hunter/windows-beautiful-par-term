...
vec3 incursionEvent(vec2 fragCoord, vec2 pixel, vec2 size, vec2 target, float targetR, float shieldStr, float railShieldStr, vec3 color) {
    ...
    float lane=clamp(target.y+side*(0.24+0.16*hash21(vec2(seed,23.0))),0.10,0.90);
      ...
      if(fragCoord.y<bandLo*size.y-bandPad || fragCoord.y>bandHi*size.y+bandPad) return color;
      // A volley only exists in the sliver between the squadron's lane and the world it is firing
      // at, so a pixel outside that sliver skips the shot geometry. The aim point stays inside the
      // world's disc or on a defender's lane, both bounded by `targetR`.
      float fireLo=min(lane,target.y-targetR)*size.y-24.0;
      float fireHi=max(lane,target.y+targetR)*size.y+24.0;
    ...
    for(int j=0;j<9;j++) {
        ...
        for(int b=0;b<3;b++) {
            if(fragCoord.y<fireLo || fragCoord.y>fireHi) continue; // firing corridor
    ...
    if(attack>0.5) {
        for(int m=0;m<3;m++) {
            ...
            for(int q2=0;q2<2;q2++) {
                if(fragCoord.y<fireLo || fragCoord.y>fireHi) continue; // same corridor
