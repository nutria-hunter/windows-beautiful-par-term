"""Third round: stub the remaining layers renderOperaScene draws, and measure each.
...
Usage: python feature_cost2.py           (writes outputs/feature-cost3.json)
"""
...
def main():
    ...
    if text is None:
        return 1
    base = measure(SOURCE, "cost3-base")
    if base is None:
    ...
    for name in TARGETS:
        ...
        if variant is None:
            ...
            continue
        path = os.path.join(SHADERS, f"cost3-{name}.glsl")
        if not write_text(path, variant):
            continue
        measured = measure(path, f"cost3-{name}")
        if measured is None:
            continue
        delta = base - measured
        rows.append({"variant": f"cost3-{name}", "stubbed": name, "gpu_ms": measured, "delta_ms": delta})
        print(f"  {name:20s} {measured:6.3f} ms   saves {delta:+6.3f} ms ({delta / base * 100:5.1f}%)")
    write_text(os.path.join(OUTPUTS, "feature-cost3.json"),
               json.dumps({"shader": SOURCE, "results": rows}, indent=2))
    print("wrote outputs/feature-cost3.json")
    return 0
