...
vec3 operaPulse(vec2 f, vec3 source, vec3 target, float age, bool planetary, vec3 color) {
    ...
    if(age<flight && z<=sceneDepth) {
        ...
        if(abs(longitudinal)<lengthPx && abs(transverse)<0.65*pixelFootprint)
            color=max(color,vec3(156,190,176)/255.0);
        if(planetary && abs(longitudinal+14.0)<8.0 && abs(transverse)<0.55*pixelFootprint)
...
void mainImage(out vec4 fragColor,in vec2 fragCoord) {
    // Preserve thin weapon cores when the background target uses fewer samples.
    vec2 footprint=fwidth(fragCoord);
    pixelFootprint=max(1.0,max(footprint.x,footprint.y));
    ...
    for(int pass=0;pass<passes;pass++) {
        ...
        coverage=1.0-smoothstep(0.20*pixelFootprint,0.80*pixelFootprint,abs(across));
    }
}
